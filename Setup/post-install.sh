#!/bin/bash
# post-install.sh - Despachador y selector inteligente de post-instalación para Arch Linux (Niri + Dank Material Shell)
...
show_help() {
    cat <<HELPEOF
Despachador de Post-Instalación para Arch Linux (Niri & Dank Material Shell)
...
Scripts independientes disponibles:
  - Setup/post-install-amd.sh   -> Optimizado para AMD Ryzen (firmware AMD, RADV, Mesa, PipeWire, Niri, DMS, Pacman)
  - Setup/post-install-intel.sh -> Optimizado para Intel Core (microcódigo Intel, VA-API Intel, PipeWire, Niri, DMS, Pacman)
HELPEOF
}
